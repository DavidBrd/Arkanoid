...
public class Level {
	...
	private void initBricksLvl1(ArrayList<Brick> bricks) {
		...
		for (int i = 0; i < COUNT_BLOCKS_X; ++i) {
			for (int j = 0; j < COUNT_BLOCKS_Y; ++j) {
				this.model.incrNbBricks();
				bricks.add( new Brick( (j+i + 1) * (Main.BRICK_WIDTH + 3) - 50,
						(j + 2) * (Main.BRICK_HEIGHT + 3) - 10, 1, Bonus.SLOW_TIME));
			}
	...
	private void lvl1(int gameMode) {
		...
		TimerTask difficulty = new Difficulty(model);
		...
		difficultyTimer.scheduleAtFixedRate(difficulty, 5_000, 5_000);
		ballSpawnTimer.scheduleAtFixedRate(ballSpawn, 8_000, 10_000);
